...
static
void atcommand_killmonster_subrogue(Session *s, dumb_ptr<map_session_data> sd,
...
static
ATCE atcommand_rogue(Session *s, dumb_ptr<map_session_data> sd,
        ZString message)
{
    if (bool(sd->status.option & Opt0::HIDE))
    {
        sd->status.option &= ~Opt0::HIDE;
        clif_displaymessage(s, "Invisible: Off."_s);
	atcommand_killmonster_subrogue(s, sd, message, 1);
    }
    else
    {
        sd->status.option |= Opt0::HIDE;
...
static
ATCE atcommand_uncamu(Session *s, dumb_ptr<map_session_data> sd,
        ZString)
{
    timestamp_seconds_buffer tsbuf;
    stamp_time(tsbuf);
...
Map<XString, AtCommandInfo> atcommand_info =
{
    ...
    {"rogue"_s, {""_s,
        40, atcommand_rogue,
        "Toggle invisibility from monsters and certain commands and kill all the mosters"_s}},
    {"die"_s, {""_s,
        40, atcommand_die,
